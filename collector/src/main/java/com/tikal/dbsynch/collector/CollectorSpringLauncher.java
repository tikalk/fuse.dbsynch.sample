 ...
 *
 */
public class CollectorSpringLauncher {

	protected static final Logger logger = LoggerFactory.getLogger(CollectorSpringLauncher.class);
...
//			Runtime.getRuntime().addShutdownHook(getShutdownHook(context));
			...
			logger.error("CollectorSpringLauncher Terminated in error:", e);
		}
	}
	...
	public static void main(String[] args) {
		...
		CollectorSpringLauncher collectorSpringLauncher = new CollectorSpringLauncher();

		if (args.length < 1) {
			logger.error("At least 1 argument are required: contextPath.");
			collectorSpringLauncher.exit(1);
		}

		String msg = "CollectorSpringLauncher started with folowing parameters:";
		...
		collectorSpringLauncher.start(contextPath);
